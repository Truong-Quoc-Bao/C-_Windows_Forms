...
namespace WindowsFormsApp_Windows_Forms_NC_Bai_3
{
    public partial class Form1 : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            this.GroupBoxChucNang.Size = new System.Drawing.Size(100, 160);
            ...
            this.ButtonSua.Location = new System.Drawing.Point(12, 90);
            this.ButtonSua.Text = "Sửa";
            this.ButtonSua.Click += new EventHandler(this.ButtonSua_Click);

            this.ButtonThoat.AutoSize = true;
            this.ButtonThoat.Location = new System.Drawing.Point(12, 120);
        ...
        private void ButtonXoa_Click(object sender, EventArgs e)
        {
            ...
            DialogResult XacNhanXoa = MessageBox.Show("Bạn có chắc chắn muốn xóa nhân viên đã chọn?", "Xác nhận xóa", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void ButtonSua_Click(object sender, EventArgs e)
        {
            if (listView1.Items.Count == 0)
            {
                MessageBox.Show("Không có nhân viên để chỉnh sửa", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            if (listView1.SelectedItems.Count == 0)
            {
            ...
            if (!isEditing)
            {
                ...
                TextBoxMNV.Text = selectedItem.SubItems[0].Text;
                TextBoxHoten.Text = selectedItem.SubItems[1].Text;
                DatetimePickerNgaySinh.Value = DateTime.Parse(selectedItem.SubItems[2].Text);
                TextBoxDiaChi.Text = selectedItem.SubItems[3].Text;

                ButtonSua.Text = "Xác nhận";
            }
            else
            {
                ListViewItem selectedItem = listView1.SelectedItems[0];
                DialogResult XacNhanSua = MessageBox.Show("Bạn có chắc muốn cập nhật thông tin nhân viên này?", "Xác nhận cập nhật", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (XacNhanSua == DialogResult.Yes)
                { 
                    selectedItem.SubItems[0].Text = TextBoxMNV.Text;
                    selectedItem.SubItems[1].Text = TextBoxHoten.Text;
                    selectedItem.SubItems[2].Text = DatetimePickerNgaySinh.Value.ToShortDateString();
                    selectedItem.SubItems[3].Text = TextBoxDiaChi.Text;

                    MessageBox.Show("Thông tin nhân viên đã được chỉnh sửa.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    isEditing = false; 
                    ButtonSua.Text = "Sửa";
                    TextBoxMNV.Clear();
                    TextBoxHoten.Clear();
